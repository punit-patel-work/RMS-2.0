...
export function UserManager({ users }: { users: UserData[] }) {
  ...
  const handleCreate = () => {
    if (!form.name.trim() || !form.pin.trim() || !form.employeeId.trim()) {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        setShowCreate(false);
        setForm({ name: "", employeeId: "", role: "FLOOR_STAFF", pin: "" });
        router.refresh();
      } else {
  ...
  const handleUpdate = () => {
    ...
    startTransition(async () => {
      ...
      if (form.pin.trim()) data.pinCode = form.pin;
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-10 h-10 rounded-full flex items-center justify-center ${rc.color}`}
                    >
